...
export type ShareFolderDialogProps = Omit<DialogProps, 'onConfirm' | 'children' | 'title'> & {
  folderId: number
  folderName: string
  /** Optional predefined share link; if not provided, a default will be generated from folderId */
  shareLink?: string
  title?: string
}
...
export default function ShareFolderDialog({
  ...
  confirmType = 'primary',
  ...dialogProps
}: Readonly<ShareFolderDialogProps>) {
  ...
  const effectiveLink = React.useMemo(
    () => shareLink ?? `${window.location.origin}/share/folder/${folderId}`,
    [folderId, shareLink],
  )
  ...
  return (
    ...
    >
      <div className="space-y-4">
        <div>
          <p className="text-sm text-gray-600 dark:text-gray-300 mb-2">Copy a link to share this folder.</p>
