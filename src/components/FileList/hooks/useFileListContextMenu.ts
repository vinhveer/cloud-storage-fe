...
import { useMockMenuItems } from '../mockMenuItems'
...
export function useFileListContextMenu({ contextMenuMode = 'default', dialogs }: UseFileListContextMenuProps) {
      ...
      return itemsWithRestore.map(item => {
        ...
        if (item.label === 'Share') {
          return {
            ...
            action: (folder: FileItem) => {
              if (!folder.id) return
              dialogs.setShareFolderDialog({ open: true, folder })
            },
          }
        }
      ...
      return itemsWithRestore.map(item => {
        ...
        if (item.label === 'Share') {
          return {
            ...
            action: (file: FileItem) => {
              if (!file.id) return
              dialogs.setShareFileDialog({ open: true, file })
            },
          }
        }
        if (item.label === 'Delete') {
