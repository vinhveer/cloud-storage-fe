...
import { useAddShareUsers } from '@/api/features/share/share.mutations'
...
export default function AddUsersForm({ shareId, shareName, onSuccess, onCancel }: AddUsersFormProps) {
  ...
  const { data: shareDetail } = useShareDetail(shareId)
  const addUsersMutation = useAddShareUsers()
  ...
  const existingUserIds = new Set(shareDetail?.shared_with.map((u) => u.user_id) || [])
  const filteredUsers = usersData?.data
    ? usersData.data.filter(
        (user) => !existingUserIds.has(user.user_id) && !selectedUsers.some((su) => su.user_id === user.user_id)
      )
    : []
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to add users. Please try again.'
