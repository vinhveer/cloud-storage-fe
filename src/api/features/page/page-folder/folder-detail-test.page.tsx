...
import { useFolderDetail } from '@/api/features/folder/folder.queries'

export default function FolderDetailTestPage() {
  const [folderId, setFolderId] = React.useState<string>('')

  const numericFolderId = folderId === '' ? undefined : Number(folderId)

  const query = useFolderDetail({
  ...
  const runQuick = (mode: 'happy' | 'not-found' | 'unauth' | 'forbidden') => {
    if (mode === 'happy') {
      setFolderId('29')
      return
    }
  ...
  return (
    <div style={{ padding: 24, fontFamily: 'sans-serif' }}>
      ...
      >
        ...
        >
          {query.isLoading ? 'Loading...' : 'Fetch folder detail'}
        </button>
      </form>
